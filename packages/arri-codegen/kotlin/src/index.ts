...
export function kotlinClientFromDef(
    ...
    for (const key of Object.keys(polymorphicClasses)) {
        ...
        for (const subClass of polymorphicClasses[key]!) {
            part += `        subclass(${subClass}::class)\n`;
        }
        part += `    }`;
        serializerModuleParts.push(part);
    }
...
export function kotlinHttpRpcFromDef(
    ...
    return `    suspend fun ${name}(${
        ...
        val error = JsonInstance.decodeFromString<${context.clientName}Error>(response.body())
        throw error
    }`;
}
...
export function kotlinPropertyFromSchema(
    ...
    if (isSchemaFormDiscriminator(schema)) {
        return kotlinSealedClassFromSchema(schema, context);
    }
...
export function kotlinSealedClassFromSchema(
