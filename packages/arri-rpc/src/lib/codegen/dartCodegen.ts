...
export function createDartClient(
    ...
    for (const service of serviceParts) {
...
export function dartServiceFromServiceDefinition(
    ...
    Object.keys(def).forEach((key) => {
        ...
        if (isProcedureDefinition(item)) {
            const returnType = item.response.length
                ? `Future<${prefix}${item.response}>`
                : "Future<String>";
            const paramsInput = item.params.length
                ? `${prefix}${item.params} params`
                : "";
            const responseParser = item.response.length
                ? `${prefix}${item.response}.fromJson(json.decode(body))`
                : null;
    ...
    return parsedRequest(
      ...
      headers: headers,
      params: ${item.params.length ? `params.toJson()` : "null"},
